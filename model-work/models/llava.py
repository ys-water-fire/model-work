...
img_path=config['data']['image_dir']
img_caption_dict={}
accum_steps=1
micro_batch=8
...
def write_llava_loss():
    ...
    optimizer=optim.AdamW(trainable,lr=config['training']['lr'],weight_decay=config['training']['weight_decay'])
    scheduler=CosineAnnealingLR(optimizer, T_max=num_epochs, eta_min=config['training']['eta_min'])
...
#checkpoint=torch.load(f'./opencv/llava_30.checkpoint.pth',map_location=device)
...
def llava_evalate(model,img_tensor,question=None,history=None):
    model.eval()
    ...
    with torch.no_grad():
        ...
        if question is not None:
          ...
          question_mask=input['attention_mask']
          vis_mask=torch.ones((b,seq_len),dtype=torch.long,device=device)
          vis_tokens=torch.cat([vis_tokens,question_embed],dim=1)
          attention_mask=torch.cat([vis_mask,question_mask],dim=1)
        else:
...
def use_LLAVA_evaluate(img_tensor):
    ...
    while True:
        ...
        answer=llava_evalate(model,image_tensor,question=user_q,history=history)
...
if __name__ == '__main__':
    img_tensor1=Image.open(config['evaluation']['test_image'])
    img_tensor2=Image.open(config['evaluation']['test_image2'])
    img_tensor3=Image.open(config['evaluation']['test_image3'])
    #use_LLAVA_evaluate(img_tensor1)
    write_llava_loss()
