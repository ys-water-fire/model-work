...
from models.common.layers import (LayerNorm, PatchEmbed, clone, Block, Causal_Mask,MultiHeadAttention,MLP,SublayerConnection)
...
def compute_recall(i2t_logits,t2i_logits,topk=(1,5,10)):
    ...
    _,sorted_indices=t2i_logits.sort(dim=-1,descending=True)
    correct_rank=(sorted_indices==correct).nonzero(as_tuple=True)[1]
...
def write_loss():
    plot_loss_list=[]
    nums_epochs=config['training']['epochs']
    model=CLIP().to(device)
    optimizer=optim.Adam(model.parameters(),lr=config['training']['lr'],weight_decay=config['training']['weight_decay'])
    criterion=nn.CrossEntropyLoss()
    scheduler=CosineAnnealingLR(optimizer,T_max=nums_epochs,eta_min=config['training']['eta_min'])
...
model=CLIP().to(device)
checkpoint=torch.load(f'./opencv/clip_20.checkpoint.pth',map_location=device)
...
if __name__ == '__main__':
    ...
    use_CLIP_evaluate(img_tensor1,img_tensor2,img_tensor3)
    test_CLIP()
