...
def BLIP2_ITM_recall():
    ...
    with torch.no_grad():
        for img_tensor,input_ids,attention_mask in get_val_dataloader():
            ...
            all_labels.append(itm_labels.cpu())
            all_probs.append(match_prob.cpu())
...
def write_BILP2_loss():
    ...
    plot_loss_list=[]
    optimizer=optim.Adam(filter(lambda p: p.requires_grad, model.parameters()),lr=config['training']['lr'],weight_decay=config['training']['weight_decay'])
    scheduler=CosineAnnealingLR(optimizer, T_max=config['training']['num_epochs'], eta_min=config['training']['eta_min'])
